...
class RouterStore<InjectedData extends object = object> implements IRouterStore<InjectedData> {
    ...
    public setPathname(extendedPathname: string) {
        if (extendedPathname === this.pathname) {
            return;
        }
        ...
        const dynamicResult = this._dynamicRoutes.map(routeDataItemToApplicationData).filter((x: IsApplicableData) => {
            if (!x.isApplicable) {
                return false;
            }
    ...
    private async canChangeView(applicableResults: IsApplicableData[]): Promise<false | CanUpdateResultObject> {
        ...
        const map = applicableResults.reduce((t, c) => {
            t[c.key] = c;
            return t;
        }, {} as Record<string, IsApplicableData>);
        ...
        const newViews: IViewStoreListItem[] = addViewKeys.map((x) => {
            ...
            if (!data.ViewStore && data.Cmp) {
                data.ViewStore = ViewStore;
            }
            if (data.ViewStore) {
                result.v = new data.ViewStore();
                if (this.injectedData) {
                    result.v.injectedData = this.injectedData;
                }
                if (data.Cmp) {
                    result.v.Cmp = data.Cmp;
                }
            }
        ...
        const canUnmount = (await Promise.all(removedViews.map((x) => x.v?.canUnmount()).filter(Boolean))).every(
            Boolean,
        );
        if (!canUnmount) {
            return false;
        ...
        if (path.includes(':')) {
            return (url: string) => {
                ...
                return {
                    ...
                    urlData: {
                        ...urlData,
                        params: { ...urlData.params, ...matches } as Record<string, string | string[]>,
                    },
                };
            };
        } else {
            ...
            if (exact) {
                return (url: string) => ({
                    isApplicable: path === url.toLowerCase(),
                    urlData: { ...toJS(this.urlData) },
                });
            } else {
                return (url: string) => ({
                    isApplicable: url.toLowerCase().startsWith(path),
                    urlData: { ...toJS(this.urlData) },
                });
            }
        }
