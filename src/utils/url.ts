...
export const endpointToRegExp = (endpoint: string, exact?: boolean): [endpointRegexp: RegExp, matchedParts: string[]] => {
    const dynamicParts = endpoint.match(dynamicRegExp);
    if (!dynamicParts) { throw new Error(`No dynamic part in "${endpoint}"`); }
...
export const isUrlMatchWithPath = (url: string, foundEndpoint: string, exact?: boolean): boolean => {
    if (!foundEndpoint.includes(':')) { return foundEndpoint === url; }
...
export const extractDataFromUrl = (url: string, foundEndpoint: string, exact?: boolean): Record<string, string> | null => {
    if (!foundEndpoint.includes(':')) { return {}; }
    ...
    if (queryList) {
        queryList.split('&').forEach(queryParams => {
            ...
            if (key.endsWith('[]') || key.endsWith('%5B%5D')) {
                ...
                if (!params[newKey]) { params[newKey] = []; }
